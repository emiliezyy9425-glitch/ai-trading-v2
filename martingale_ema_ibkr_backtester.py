# =============================================================================
# FINAL FIXED VERSION – Real Daily EMA10 Crossovers Only (Previous Day)
...
async def get_previous_day_ema10(ib: IB, contract: Stock) -> pd.Series:
    """Return EMA10 using ONLY previous completed day's value"""
    ...
    # ONE LINE FIX: Only yesterday's EMA10 is known today
    return df["ema10"].resample("1D").last().shift(1)


async def run_backtest(symbol: str, timeframe: str):
    print(f"\n{'=' * 80}")
    print(f"REAL TEST → {symbol} | {timeframe} | Previous-Day EMA10 Only")
    print(f"{'=' * 80}")

    ib = connect_ibkr(max_retries=3, initial_client_id=300)
    ...
    # === FIXED: Real previous-day EMA10 ===
    ib_daily = connect_ibkr(max_retries=1, initial_client_id=310)
    if ib_daily and ib_daily.isConnected():
        try:
            daily_ema = await get_previous_day_ema10(ib_daily, contract)
            df["ema10"] = daily_ema.reindex(df.index, method="ffill")
            df["prev_ema10"] = daily_ema.shift(1).reindex(df.index, method="ffill")
        finally:
            ib_daily.disconnect()
    else:
        df["ema10"] = np.nan
        df["prev_ema10"] = np.nan

    # === REAL SIGNALS – NO LOOK-AHEAD ===
    df["buy_signal"]  = (df["close"].shift(1) <= df["prev_ema10"]) & (df["close"] > df["ema10"])
    df["sell_signal"] = (df["close"].shift(1) >= df["prev_ema10"]) & (df["close"] < df["ema10"])

    # === Your original Martingale engine (unchanged) ===
    equity = CAPITAL
    multiplier = 1.0
    position = 0
    entry_price = 0.0
    ...
    for i in range(1, len(df)):
        ...
        # Entry
        if position == 0 and pd.notna(row.ema10):
            if row.buy_signal:
                position = 1
    ...
    # === Your original result saving (100% unchanged) ===
    ...
    # Save logs exactly like before
    if not log_df.empty:
        csv_file = Path("/host_desktop") if Path("/host_desktop").exists() else DATA_DIR
        csv_file = csv_file / "martingale_summary_all_timeframes.csv"
...
async def main():
    ...
    print(f"Testing {len(tickers)} symbols on ALL 9 timeframes with REAL crossovers only...\n")
    ...
    print("\nALL DONE! Check martingale_summary_all_timeframes.csv on your desktop.")
