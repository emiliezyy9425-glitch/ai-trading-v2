# =============================================================================
# 10-Day VWMA (Close) Reversion + Pure Martingale Backtester (IBKR Ready)
...
def get_daily_vwma10(ib: IB, contract: Stock, end_date: datetime) -> pd.DataFrame:
    """Fetch daily bars and compute a non-repainting 10-day VWMA on close price."""
    ...
    rolling_pv = price_volume.rolling(window=10).sum()
    rolling_volume = df["volume"].rolling(window=10).sum()
    df["vwma10"] = rolling_pv / rolling_volume
    return df[["date", "close", "vwma10"]].set_index("date")
...
async def run_backtest(symbol: str, timeframe: str) -> pd.DataFrame:
    ...
    # === FIXED VWMA10 WITH PROPER DATETIMEINDEX ===
    ...
    if ib_daily and ib_daily.isConnected():
        try:
            ...
            rolling_pv = price_volume.rolling(window=10).sum()
            rolling_volume = daily_df["volume"].rolling(window=10).sum()
            ...
            if pd.Timestamp.now(tz="UTC").date() > latest_daily_date.date():
                current_indicator = daily_df["vwma10"].iloc[-1]
            else:
                current_indicator = daily_df["vwma10"].iloc[-2]

            df["vwma10"] = current_indicator
    ...
    df["prev_vwma"] = df["vwma10"].shift(1)

    df["buy_signal"] = (df["prev_close"] < df["prev_vwma"]) & (df["close"] > df["vwma10"])
    df["sell_signal"] = (df["prev_close"] > df["prev_vwma"]) & (df["close"] < df["vwma10"])
    ...
    for i in range(1, len(df)):
        ...
        # New entry
        if position == 0 and not pd.isna(row.vwma10):
            if row.buy_signal:
                position = 1
