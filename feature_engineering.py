    ...
    def _process_group(group: pd.DataFrame) -> pd.DataFrame:
        ...
        for suffix, meta in optional_timeframes.items():
            ...
            if suffix == "4h":
                ...
                daily_price = resampled.set_index("timestamp")[price_col]
                hourly_price = local.set_index("timestamp")["price_1h"]

                resampled["ret_4h_1d"] = np.log(
                    daily_price
                    / hourly_price.shift(4)
                    .resample(rule)
                    .last()
                    .reindex(daily_price.index)
                ).values
                resampled["ret_24h_1d"] = np.log(
                    daily_price
                    / hourly_price.shift(24)
                    .resample(rule)
                    .last()
                    .reindex(daily_price.index)
                ).values
