# =============================================================================
# 10-Day EMA (Close) Reversion + Pure Martingale Backtester (IBKR Ready)
...
def get_daily_ema10(ib: IB, contract: Stock, end_date: datetime) -> pd.DataFrame:
    """Fetch daily bars and compute a non-repainting 10-day EMA on close price."""
...
async def run_backtest(symbol: str, timeframe: str) -> pd.DataFrame:
    ...
    for i in range(1, len(df)):
        ...
        # New entry
        if position == 0 and not pd.isna(row.ema10):
            if row.buy_signal:
                position = 1
